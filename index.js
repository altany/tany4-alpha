...
app.use(
  ...
  express.static(
    path.join(__dirname, '/public/files/TaniaPapazafeiropoulouCV.pdf')
  )
)
...
app.get('/:page?', function (req, res, next) {
  ...
  visitor
    .pageview({
      dp: '/' + (req.params.page ? req.params.page : ''),
      dt: page.charAt(0).toUpperCase() + page.slice(1)
    })
    .send()

  // If page is Github, render the Github API results
  if (req.params.page && req.params.page === 'github') {
    request.get('http://' + req.headers.host + '/api/github/repos', function (
      error,
      response,
      body
    ) {
      ...
      if (repos.length) {
        async.each(
          ...
          function (repo, callback) {
            request.get(
              'http://' + req.headers.host + '/api/github/readme/' + repo.name,
                ...
                request.get(
                  ...
                  function (e, r, b) {
                    if (e || r.statusCode !== 200) {
                      repo.lastCommit = {
                        message: 'Error getting the last commit info...'
                      }
                      console.error(
                        'Error get the last commit for ' + repo.name
                      )
                    } else {
          ...
          function (err) {
            if (err) {
              console.error(
                'Async failed while reading one of the repos details'
              )
            }
    ...
    // Not Github
    res.render(req.params.page ? req.params.page : 'index', {
      page: page,
      ldJson: ldJson
    })
  }
